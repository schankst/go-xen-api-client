...
const (
	...
	VdiOperationsRevertTo VdiOperations = "revert_to"
	// Reverting this VDI to a clone of a snapshot
	VdiOperationsRevertFrom VdiOperations = "revert_from"
)
...
func (_class VDIClass) Revert(sessionID SessionRef, snapshot VDIRef) (_err error) {
	...
	_snapshotArg, _err := convertVDIRefToXen(fmt.Sprintf("%s(%s)", _method, "snapshot"), snapshot)
	if _err != nil {
		return
	}
