...
This is a private fork of github.com/terra-farm/go-xen-api-client,
regenerated against a current XenAPI schema (see SchemaXAPIRelease) instead
of upstream's last release, which only covered XenAPI through XenServer
7.3. See the README for what else differs from upstream, and GitHub
Releases (https://github.com/schankst/go-xen-api-client/releases) for
...
one Go type per XenAPI class/enum - see the README's "Automated
regeneration" section for how and when that happens. error.go (XenAPI error
code constants) is regenerated from a separate upstream source on the same
schedule. client.go, xenapi.go (the generator itself), and the xmlrpc
subpackage are hand-maintained.
*/
package xenapi
